  ...
  ****************************************************************************/
  ...
  sendSms: function(opts, callback) {
    ...
    this._makeRequest(this._baseUrl + "/v3/sendsms", "POST", params, function(error, response) {
      ...
      if (error == null) {
        var parsed_response = {tx_guid: response.txguid, sms_parts: response.sms_parts, numbers: response.numbers, encoding: response.encoding, cost_in_pence: response.cost_in_pence, new_balance_in_pence: response.new_balance_in_pence};
