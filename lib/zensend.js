...
Client.DEFAULT_VERIFY_URL = "https://verify.zensend.io";
...
function Client(key, opts) {
  ...
  this._verifyUrl = opts["verify_url"] || Client.DEFAULT_VERIFY_URL;
...
Client.prototype = {
  ...
  createMsisdnVerification : function(msisdn, verify_options, callback) {
    var http_params = {"NUMBER": msisdn};

    if (verify_options != null) {
      if (verify_options["message"] != null) {
        http_params["MESSAGE"] = verify_options["message"];
      }

      if (verify_options["originator"] != null) {
        http_params["ORIGINATOR"] = verify_options["originator"];
      }
    }

    this._makeRequest(this._verifyUrl + "/api/msisdn_verify", "POST", http_params, function(error, response) {
      if (error == null) {
  ...
  msisdnVerificationStatus: function(session, callback) {
    var http_params = {"SESSION" : session};

    var qs = querystring.stringify(http_params);

    this._makeRequest(this._verifyUrl + "/api/msisdn_verify?" + qs, "GET", http_params, function(error, response) {
      if (error == null) {
        callback(null, response["msisdn"]);
      } else {
        return callback(error, response);
      }
    });      
  ...
  ****************************************************************************/
  ...
  createSubAccount: function(name, callback) {

    this._makeRequest(this._baseUrl + "/v3/sub_accounts", "POST", {NAME: name}, callback);
  },
